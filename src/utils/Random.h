...
class Random {
 public:
  Random(int seed = 1) { mt_rand.seed(seed); }
  ...
  OUTPUT_TYPE weightedRandom(const std::vector<std::pair<float, OUTPUT_TYPE>> &probabilities_) {
    ...
    float proba_sum =
        std::accumulate(probabilities.begin(), probabilities.end(), 0.0,
                        [](float sum, const std::pair<float, OUTPUT_TYPE> &pair) { return sum + pair.first; });
    if (proba_sum > 1) {
      throw std::invalid_argument("weightedRandom cannot have probabilities sum above 1");
    }
    // sort the probabilities
    std::sort(probabilities.begin(), probabilities.end(),
              [](const std::pair<float, OUTPUT_TYPE> &a, const std::pair<float, OUTPUT_TYPE> &b) {
