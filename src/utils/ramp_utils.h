...
inline double linearRamp(double x, const RampParameters &parameters) {
  ...
  if (x < parameters.transitional_start) {
    ...
    double steep = (parameters.final_value - parameters.initial_value) /
                   (parameters.transitional_end - parameters.transitional_start);
...
inline double linearRampIntegral(double x, const RampParameters &parameters) {
  ...
  if (x < parameters.transitional_start) {
    ...
    if (x < parameters.transitional_end) {
      ...
      return initial_aire + affineIntegral(steep, parameters.initial_value, transitional_dx) +
             parameters.final_value * final_dx;
    }
  }
}
}   // namespace ramp_utils