...
Floating lin_interp(const Floating &x, const std::vector<Floating> x_values, const std::vector<Floating> y_values,
  ...
  if (lower_it == x_values.begin()) {
    ...
    const double steep =
        (y_values[second_index] - y_values[first_index]) / (x_values[second_index] - x_values[first_index]);
...
template <typename FUNCTION, typename Floating, typename = std::enable_if_t<std::is_floating_point<Floating>::value>>
