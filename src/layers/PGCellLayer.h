...
{
public:
    PGCellLayer(std::unique_ptr<PGCellsModel> &pgc_model_ptr, const ConeLayer &cone_layer, int seed = 1) : pgc_model_ptr_(std::move(pgc_model_ptr)), random(seed)
    {
        ...
        std::cout << "Pix parasol cells max eccentricity: " << pgc_model_ptr_->getEccentricityAt(cellsWidth / 2) << " °" << std::endl;
        std::cout << "parasol dimensions: " << cells_cpu_.width << " x " << cells_cpu_.height << std::endl; //" pgc_model_ptr_->getTotalRadius()
        ...
        for (int j = 0; j < cells_cpu_.height; j++)
        {
            for (int i = 0; i < cells_cpu_.width; i++)
            {
                ...
                cv::Point src_pos = polar_utils::getPosition(parasol_central_cone, cv::Size(cones_cpu.width, cones_cpu.height), direction);
                ...
                else if (j == cells_cpu_.height / 2)
                {
                    ...
                    description_map_["pgc_cones_in_receptive_at_half"].push_back(M_PI * ganglionarExternalRadius * ganglionarExternalRadius / UNIT_DISK_SURFACE);
                    ...
                    description_map_["gcp_cone_ratio"].push_back(pgc_model_ptr_->getDensityAt(parasol_angular_pose) / cone_model.getDensityAt(parasol_angular_pose));
    ...
    const GanglionarCells &pgcells() const
    {
        return cells_cpu_;
    }
