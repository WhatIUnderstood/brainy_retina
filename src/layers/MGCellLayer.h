...
/**
 * @brief Magno cellular cells layer
 *
 */
class MGCellLayer {
 public:
  MGCellLayer(std::unique_ptr<MGCellsSim> &mgc_model_ptr, const ConeLayer &cone_layer, int seed = 1)
      : mgc_model_ptr_(std::move(mgc_model_ptr)), random(seed) {
    ...
    std::cout << "Pix midget cells max eccentricity: " << mgc_model_ptr_->getEccentricityAt(cellsWidth / 2) << " °"
              << std::endl;
    std::cout << "midget dimensions: " << cells_cpu_.width << " x " << cells_cpu_.height
              << std::endl;   //" mgc_model_ptr_->getTotalRadius()
    ...
    for (int j = 0; j < cells_cpu_.height; j++) {
      for (int i = 0; i < cells_cpu_.width; i++) {
        ...
        cv::Point src_pos =
            polar_utils::getPosition(midget_central_cone, cv::Size(cones_cpu.width, cones_cpu.height), direction);
        ...
        if (j == cells_cpu_.height / 8) {
          ...
          description_map["gcm_cone_ratio"].push_back(mgc_model_ptr_->getDensityAt(midget_angular_pose) /
                                                      cone_model.getDensityAt(midget_angular_pose));
  ...
  const GanglionarCells &mgcells() const { return cells_cpu_; }
