...
/**
 * @brief Cones photoreceptors layer
 *
 */
class ConeLayer {
 public:
  /**
   * @brief Construct a new Cone Layer object
   *
   * @param cone_model_ptr
   * @param pixel_model_ptr
   * @param seed
   */
  ConeLayer(std::unique_ptr<ConeModel> &cone_model_ptr, std::unique_ptr<PixelConeModel> &pixel_model_ptr, int seed = 1)
      : cone_model_ptr_(std::move(cone_model_ptr)), pixel_model_ptr_(std::move(pixel_model_ptr)), random(seed) {
    ...
    std::cout << "Cone model max eccentricity     : "
              << cone_model_ptr_->getEccentricityAt(cone_model_ptr_->getMaxIndex()) << " °" << std::endl;
    ...
    for (int j = 0; j < cone_height; j++) {
      for (int i = 0; i < cone_width; i++) {
        ...
        if (pixel_index < 0 || src_pos.x > input_width || src_pos.y > input_height || src_pos.x < 0 || src_pos.y < 0) {
          cone.type = PHOTO_TYPE::NONE;
        } else {
          cone.center_x = src_pos.x;
          cone.center_y = src_pos.y;
          cone.type =
              random.weightedRandom<PHOTO_TYPE>({{cone_model_ptr_->config().ph_S_cone_ratio, PHOTO_TYPE::S_CONE},
                                                 {cone_model_ptr_->config().ph_M_cone_ratio, PHOTO_TYPE::M_CONE},
                                                 {cone_model_ptr_->config().ph_L_cone_ratio, PHOTO_TYPE::L_CONE}});
  ...
  const ConeModel &coneModel() const { return *cone_model_ptr_; }

  const PixelConeModel &pixelModel() const { return *pixel_model_ptr_; }

  void plotGraphs();

  const Cones &cones() const { return cones_cpu_; }
